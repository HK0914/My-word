...
color_mask = imread('alice_mask.png')  #设置背景图

# 绘制词云 参数 font_path 字体， background 背景色，max_words 最大词数，mask 形状
cloud = WordCloud(
...
#构建多元回归模型
y = df_with_dummy['月工资']
X = df_with_dummy[['经验','学历_大专','学历_本科']]
X=sm.add_constant(X)
model = sm.OLS(y,X)
results = model.fit()
